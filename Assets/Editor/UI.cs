...
public class UI: EditorWindow
{
    ...
    private static Sprite LoadSpriteFromResources(string resourcePath)
    {
        ...
        // Fallback for legacy paths where the folder was omitted.
        if (!resourcePath.StartsWith("UI/BuffIcons", StringComparison.OrdinalIgnoreCase)) {
            sprite = Resources.Load<Sprite>($"UI/BuffIcons/{resourcePath}");
            if (sprite != null) {
                return sprite;
            }
        }
    ...
    public void addAttribute(Game.Attribute attribute) {
