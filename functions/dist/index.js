...
const functions = __webpack_require__(701);
const admin = __webpack_require__(675);
const {
  CloudTasksClient
...
async function ensureQueue(projectId) {
  ...
  try {
    const [existingQueue] = await tasksClient.getQueue({
      name: queuePath
    });
    const needsUpdate = !existingQueue.rateLimits || existingQueue.rateLimits.maxDispatchesPerSecond !== desiredRateLimits.maxDispatchesPerSecond || existingQueue.rateLimits.maxConcurrentDispatches !== desiredRateLimits.maxConcurrentDispatches;
    if (needsUpdate) {
      ...
      functions.logger.info("Updated Cloud Tasks queue rate limits", {
        queuePath,
        desiredRateLimits
      });
    }
    ...
    if (error.code === 5) {
      ...
      functions.logger.info("Created Cloud Tasks queue with custom rate limits", {
        queuePath,
        desiredRateLimits
      });
    } else {
...
function validateChunk(chunk, chunkSize) {
  ...
  if (tiles.length > MAX_TILES_PER_CHUNK) {
    throw new functions.https.HttpsError("invalid-argument", `Chunk '${id}' exceeds the maximum supported tile count of ${MAX_TILES_PER_CHUNK}.`);
  }
  for (const tile of tiles) {
    ...
    if (tileName !== null && typeof tileName !== "string" || tileObjectPath !== null && typeof tileObjectPath !== "string") {
      throw new functions.https.HttpsError("invalid-argument", `Chunk '${id}' contains tiles with invalid tileName or tileObjectPath values.`);
    }
  }
...
exports.saveMap = functions.region("us-central1").runWith({
  timeoutSeconds: 540,
  memory: "1GB"
  ...
  try {
    ...
    if (chunks.length > MAX_CHUNK_DOCS) {
      throw new functions.https.HttpsError("invalid-argument", `Too many chunk documents (${chunks.length}). Maximum supported is ${MAX_CHUNK_DOCS}.`);
    }
    ...
    if (chunkIdsOverride !== undefined) {
      ...
      for (const id of chunkIdsOverride) {
        ...
        if (chunkIdsOverrideSet.size > MAX_CHUNK_DOCS) {
          throw new functions.https.HttpsError("invalid-argument", `Too many chunk ids (${chunkIdsOverrideSet.size}). Maximum supported is ${MAX_CHUNK_DOCS}.`);
        }
      }
    }
    ...
    if (targetChunkIdSet.size > MAX_CHUNK_DOCS) {
      throw new functions.https.HttpsError("invalid-argument", `Too many chunk ids (${targetChunkIdSet.size}). Maximum supported is ${MAX_CHUNK_DOCS}.`);
    }
    ...
    async function flushPending(reason) {
      ...
      functions.logger.debug("Flushing task batch", {
        mapId,
        count,
        reason
      });
    ...
    for (const {
      ...
      if (!tiles.length) {
        ...
        pendingTaskPromises.push(tasksClient.createTask({
          parent: queuePath,
          task: {
            httpRequest: {
              httpMethod: "POST",
      ...
      for (let start = 0; start < tiles.length; start += TILES_PER_TASK) {
        ...
        pendingTaskPromises.push(tasksClient.createTask(task).then(() => {
          tasksCreated += 1;
    ...
    await Promise.all([mapRef.set(mapDocument, {
      merge: true
    }), mapDataRef.set(mapDataDocument, {
      merge: true
    })]);
    functions.logger.info(`Queued ${tilesScheduled} tiles across ${tasksCreated} tasks for map '${mapId}' (chunks this batch: ${sanitizedChunks.length}).`, {
      mapId,
      tasksCreated,
      tilesScheduled,
...
exports.processMapTileTask = functions.region("us-west2").runWith({
  timeoutSeconds: 120,
  memory: "8GB"
  ...
  if (typeof mapId !== "string" || mapId.trim().length === 0) {
    res.status(400).json({
      error: "mapId must be provided."
    });
    return;
  }
  if (typeof chunkId !== "string" || chunkId.trim().length === 0) {
    res.status(400).json({
      error: "chunkId must be provided."
    });
    return;
  }
  const mapDataRef = firestore.collection("MapData").doc(mapId);
